# Schémas Mermaid pour les Commandes Git
...
flowchart TD
    ...
    B -->|--local| D[.git/config<br/>Configuration du projet]
    B -->|--system| E[/etc/gitconfig<br/>Configuration système]
...
flowchart TD
    ...
    B --> C[stash@{0}: message1]
    B --> D[stash@{1}: message2]
    B --> E[stash@{2}: message3]
    style B fill:#9c27b0
```
...
```mermaid
flowchart LR
    A[Stash Stack] -->|git stash drop stash@{1}| B[Supprime stash spécifique]
...
```mermaid
flowchart LR
    A[ancien_nom.txt] -->|git mv ancien_nom.txt nouveau_nom.txt| B[nouveau_nom.txt]
...
flowchart TD
    ...
    B --> C[HEAD@{0}: commit message]
    B --> D[HEAD@{1}: checkout branch]
    B --> E[HEAD@{2}: reset --hard]
    style B fill:#9c27b0
```
